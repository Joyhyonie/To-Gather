...
public class Pagenation {
    ...
    public static SelectCriteria getSelectCriteria(int page, int totalCount, int limit, int buttonAmount, Map<String, String> searchMap) {
        ...
        /* 현재 페이지에 보여줄 시작 페이지 수(올림처리) */
        startPage = (int) (Math.ceil((double) page / buttonAmount) - 1) * buttonAmount + 1;

        /* 목록 아래쪽에 보여질 마지막 페이지 수  */
        endPage = startPage + buttonAmount - 1; 
        // startPage가 6이고 buttonAmount(한 번에 보여질 페이징 버튼의 갯수)가 5일 경우 endPage는 10. 따라서 6부터 10 페이징 버튼이 노출
        ...
        /* 마지막 페이지는 0이 될 수 없기 때문에 게시물이 아무 것도 존재하지 않으면 max페이지와 endPage는 0 */
    ...
    /* 사진 게시판 페이징을 위해 메소드 추가 생성 (검색어가 존재하지 않는 경우) */
	public static SelectCriteria getSelectCriteria(int page, int totalCount, int limit, int buttonAmount) {

		return getSelectCriteria(page, totalCount, limit, buttonAmount, new HashMap<>()); /* 현재 new HashMap<>()는 빈 값 (검색어 X) */
	}
}
